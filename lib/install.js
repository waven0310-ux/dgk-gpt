...
function printUsage() {
  console.log(`
Usage:
  npx dgk-gpt [--yes] [--dry-run] [--skills-dir auto|user|legacy] [--with-pix-devtools]

Options:
  --yes                 Skip the confirmation prompt.
  --dry-run             Show what would change without writing files.
  --skills-dir <mode>   auto (default), user (~/.agents/skills), legacy (~/.codex/skills)
  --with-pix-devtools   Install optional WSL Pix DevTools launchers and MCP config.
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (value === "--with-pix-devtools") {
      options.withPixDevtools = true;
      continue;
    }
    if (value === "--skills-dir") {
...
function detectPixDevtoolsInstalled(homeDir) {
  return (
    existsSync(path.join(homeDir, ".local", "bin", "chrome")) ||
    existsSync(path.join(homeDir, ".local", "bin", "tauri-pix")) ||
    existsSync(path.join(homeDir, ".config", "powershell", "tauri-dev.ps1"))
  );
}
...
function buildManagedConfig(runtime, options) {
  const enablePixDevtools = Boolean(options.withPixDevtools);
  const enableSerena = runtime.hasUvx;
  ...
  const mcpServers = new Map([
    ...
    ["mcp_servers.serena", [
      ...
      `enabled = ${enableSerena ? "true" : "false"}`,
    ...
    ["mcp_servers.chrome-devtools", [
      ...
      `enabled = ${enablePixDevtools ? "true" : "false"}`,
    ]],
    ["mcp_servers.jina", [
  ...
  if (enablePixDevtools) {
    mcpServers.set("mcp_servers.tauri-devtools", [
      'command = "npx"',
      'args = ["-y", "chrome-devtools-mcp@latest", "--browser-url=http://127.0.0.1:9334"]',
      "enabled = true",
    ]);
  }
...
function mergeConfig(runtime, options, existingContent) {
  let nextContent = existingContent ?? "";
  if (!nextContent.trim()) {
    nextContent = "# Managed in part by dgk-gpt. Existing custom sections are preserved.\n";
  }

  const { mcpServers, profiles } = buildManagedConfig(runtime, options);
...
function buildManagedAgents(options) {
  const sections = [
    readFileSync(path.join(packageRoot, "templates", "AGENTS.md"), "utf8").trim(),
  ];

  if (options.withPixDevtools) {
    sections.push(
      readFileSync(path.join(packageRoot, "templates", "AGENTS.pix-devtools.md"), "utf8").trim(),
    );
  }
...
function collectPlans(runtime, options, homeDir) {
  const resolvedOptions = {
    ...options,
    withPixDevtools: options.withPixDevtools || detectPixDevtoolsInstalled(homeDir),
  };
  ...
  const powershellConfigDir = path.join(homeDir, ".config", "powershell");
  const legacyAwareSkillsRoot = resolveLegacyAwareSkillsRoot(homeDir, resolvedOptions.skillsDirMode);
  ...
  const scripts = [
    ...
    {
      action: actionForFile(
        path.join(packageRoot, "assets", "codex-scripts", "codex-statusline.py"),
        path.join(codexScriptsDir, "codex-statusline.py"),
      ),
      source: path.join(packageRoot, "assets", "codex-scripts", "codex-statusline.py"),
      target: path.join(codexScriptsDir, "codex-statusline.py"),
    },
    {
      action: actionForFile(
  ...
  if (resolvedOptions.withPixDevtools) {
    scripts.push(
      {
        action: actionForFile(
          path.join(packageRoot, "assets", "pix-devtools", "bin", "chrome"),
          path.join(localBinDir, "chrome"),
        ),
        source: path.join(packageRoot, "assets", "pix-devtools", "bin", "chrome"),
        target: path.join(localBinDir, "chrome"),
      },
      {
        action: actionForFile(
          path.join(packageRoot, "assets", "pix-devtools", "bin", "tauri-pix"),
          path.join(localBinDir, "tauri-pix"),
        ),
        source: path.join(packageRoot, "assets", "pix-devtools", "bin", "tauri-pix"),
        target: path.join(localBinDir, "tauri-pix"),
      },
      {
        action: actionForFile(
          path.join(packageRoot, "assets", "pix-devtools", "powershell", "tauri-dev.ps1"),
          path.join(powershellConfigDir, "tauri-dev.ps1"),
        ),
        source: path.join(packageRoot, "assets", "pix-devtools", "powershell", "tauri-dev.ps1"),
        target: path.join(powershellConfigDir, "tauri-dev.ps1"),
      },
    );
  }
  ...
  return {
    ...
    officialSkillsRoot,
    paths: { agentsDir, codexDir, codexScriptsDir, localBinDir, powershellConfigDir },
    runtime,
    scripts,
    skills,
    deprecatedSkills,
...
function describePlan(plans) {
  ...
  dim(`serena MCP: ${plans.runtime.hasUvx ? "enabled (uvx detected)" : "disabled (uvx not found)"}`);
  dim(`tmux helper: ${plans.runtime.hasTmux ? "ready" : "tmux missing"}`);
  if (plans.options.withPixDevtools) {
    info(`pix devtools addon: enabled${plans.runtime.isWsl ? "" : " (WSL2/Windows helper files only)"}`);
  }
...
function applyInstall(plans) {
  ...
  const agentsTemplate = buildManagedAgents(plans.options);
  ...
  const nextConfig = mergeConfig(plans.runtime, plans.options, beforeConfig);
  ...
  console.log("");
  info(`원클릭 상태: 핵심 Codex 셋업 완료${plans.runtime.hasUvx ? "" : " (serena는 uvx 설치 전까지 비활성화)"}`);
  info("다음 단계:");
  info("  1. Codex를 재시작해서 스킬/설정 목록을 새로 읽게 하세요.");
  info("  2. Codex가 아직 없다면: bash ~/.local/bin/setup-codex.sh");
  if (!plans.runtime.hasUvx) {
    info("  3. Serena MCP까지 쓰려면 uv/uvx를 설치한 뒤 dgk-gpt를 한 번 더 실행하세요.");
    if (!plans.runtime.hasTmux) {
      info("  4. tmux까지 쓰려면: bash ~/.local/bin/setup-codex.sh --install-tmux --yes");
      info("  5. /rr, /rrr까지 쓰려면: npm install -g glm-review  그리고 ZAI_API_KEY 설정");
      info("  6. Jina MCP를 쓰려면 JINA_API_KEY를 export 한 뒤 ~/.codex/config.toml에서 enabled=true 로 바꾸세요.");
    } else {
      info("  4. /rr, /rrr까지 쓰려면: npm install -g glm-review  그리고 ZAI_API_KEY 설정");
      info("  5. Jina MCP를 쓰려면 JINA_API_KEY를 export 한 뒤 ~/.codex/config.toml에서 enabled=true 로 바꾸세요.");
    }
  } else {
    if (!plans.runtime.hasTmux) {
      info("  3. tmux까지 쓰려면: bash ~/.local/bin/setup-codex.sh --install-tmux --yes");
      info("  4. /rr, /rrr까지 쓰려면: npm install -g glm-review  그리고 ZAI_API_KEY 설정");
      info("  5. Jina MCP를 쓰려면 JINA_API_KEY를 export 한 뒤 ~/.codex/config.toml에서 enabled=true 로 바꾸세요.");
    } else {
  ...
  if (plans.options.withPixDevtools) {
    ...
    info(`  ${pixStep}. Pix DevTools launcher: chrome  /  tauri-pix`);
  }
}
