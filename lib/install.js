...
function error(message) {
  console.error(`${colors.red}[dgk-gpt]${colors.reset} ${message}`);
}
...
function resolveLegacyAwareSkillsRoot(homeDir, mode) {
  ...
  const legacySignal = legacyAwareSkillNames.some((skillName) =>
    existsSync(path.join(legacySkillsDir, skillName, "SKILL.md"))
  );
...
function buildManagedConfig(runtime) {
  ...
  const featureEntries = buildFeatureMap(
    runtime,
    {},
  );

  return { featureEntries, mcpServers, profiles };
}
...
function mergeConfig(runtime, existingContent) {
  ...
  const { featureEntries, mcpServers, profiles } = buildManagedConfig(runtime);
...
function collectPlans(runtime, options, homeDir) {
  ...
  const skills = [];
  for (const skillName of legacyAwareSkillNames) {
...
function applyInstall(plans) {
  ...
  if (backups.length > 0) {
    info(`백업 위치: ${relativeToHome(plans.homeDir, path.dirname(path.dirname(backupDir)))}/${path.basename(backupDir)}`);
  }
